...
//	@FXML private TextField txtLevel;
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		// 17. 날짜 선택 이벤트 처리	(날짜 선택되면 날짜를 출력하는 텍스트에 바로 출력해주는 이벤트처리)			
		dpDate.setOnAction(e-> {
			handlerDpDateAction(e);				
		});
		//	18. 이미지 버튼 선택이벤트
	      btnImageFile.setOnAction(event ->{
	    	  handlerBtnImageFileAction(event);
	    	  }); // 이미지선택창
...
//		txtMusic.clear();
	...
	// 17. 날짜 선택 이벤트 처리	(날짜 선택되면 날짜를 출력하는 텍스트에 바로 출력해주는 이벤트처리)	
	private void handlerDpDateAction(ActionEvent e) {
			LocalDate date = dpDate.getValue();//날자를 누를때마다 값을 찍어준다.
			txtDay.setText("" + date);
	}
	
	//	18. 이미지 버튼 선택이벤트
	private Object handlerBtnImageFileAction(ActionEvent event) {
		
		return null;
	}
		
	public String imageSave(File selectedFile2) {
		
		return null;
	}
	
	/***********************************************************
	 * imageDelete() 이미지 삭제 메소드. 19.10.11 by 이태성 
	 * @param (삭제할 파일명)
	 * @return 삭제여부를 리턴
	 ***********************************************************/
...
//		txtLevel.setDisable(c);
	...
	public void handlerBtnSumAction() {
